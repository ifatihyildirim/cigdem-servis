...
/** Marka işareti: gösterge + "Ç" + sedil (oto temalı monogram). */
export function Brandmark({ className = "h-9 w-9" }: { className?: string }) {
  return (
    <svg viewBox="0 0 64 64" className={className} aria-label="Çiğdem Servis">
      <defs>
        <linearGradient id="cs-mark" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0" stopColor="#2f6bff" />
          <stop offset="1" stopColor="#0c2747" />
        </linearGradient>
      </defs>
      <rect width="64" height="64" rx="15" fill="url(#cs-mark)" />
      <path
        d="M46.74 21.67 A18 18 0 1 0 46.74 42.33"
        fill="none"
        stroke="#fff"
        strokeWidth="6.5"
        strokeLinecap="round"
...
function Logo({ size = "h-9 w-9" }: { size?: string }) {
  return <Brandmark className={size} />;
}
